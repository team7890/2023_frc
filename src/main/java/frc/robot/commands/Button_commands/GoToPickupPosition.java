...
public class GoToPickupPosition extends CommandBase {
  ...
  double dForearmTarget = 60.1;
  double dWristTarget = 8.5;

  /** Creates a new ScoreCubeTop. */
  public GoToPickupPosition(Arm_subsystem objArm_in, Forearm_subsystem objForearm_in, Wrist_subsystem objWrist_in) {
    objArm = objArm_in;
  ...
  @Override
  public void execute() {
    if (objForearm.getForearmAngle() < 10.0) {
      dArmCommand_old = objArm.moveArmToAngle(-5.0, dArmAngle_old, dArmCommand_old, 3.0);
      ...
      if (objWrist.getWristAngle() > 0.0) {
        dWristCommand_old = objWrist.moveWristToAngle(35.0, dWristAngle_old, dWristCommand_old, 2.0);
      }
      else {
